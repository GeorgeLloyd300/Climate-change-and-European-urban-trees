...
The text and code below summarises a workflow in R that can be used to relatively rapidly assess the environmental range of a species ::
...
This code is being developed at uni of x 
...
The species list is supplied to a series of functions to calculate environmental ranges and habitat suitability. The initial functions download all species records from the the Global Biodiversity Information Facility (GBIF, https://www.gbif.org/). The species data are downloaded as individual .Rdata files to the specified folders, which must exist first, without returning anything.
