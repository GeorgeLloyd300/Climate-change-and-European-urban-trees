...
for(i in 1:nrow(city.data)) { 
  ...
  city.niches<-
    new.niches %>% select(searchTaxon, Precip_dry_month_q05 , Precip_dry_month_q95)%>% # this line should be changed when using different threshold (Precip_dry_month_q02 to Precip_dry_month_q05 and Precip_dry_month_q95 to Precip_dry_month_q98) 
    subset(searchTaxon %in% city.sp) %>% mutate(city = name) %>%
    rename(species =searchTaxon, lower = Precip_dry_month_q05, upper= Precip_dry_month_q95)%>% # this line should be changed when using different threshold (Precip_dry_month_q02 to Precip_dry_month_q05 and Precip_dry_month_q95 to Precip_dry_month_q98) 
...
# save
write.csv(master.niches, "master.niches.prec.5_95.csv")
...
master.niches<-read.csv("master.niches.prec.5_95.csv")
